...
```{r}
# Read and clean renew-use-xxxx.csv files
renew_2021 <- read.csv("data/renew-use-2021.csv") |>
    rename(state = State, source = Energy_Source, usage = Renewable_Use_2021) |>
    mutate(year = 2021, usage = as.numeric(str_extract(usage, "\\d+")))
 
renew_2022 <- read.csv("data/renew-use-2022.csv") |>
    rename(state = State, source = Energy_Source, usage = Renewable_Use_2022) |>
    mutate(year = 2022, usage = as.numeric(str_extract(usage, "\\d+")))
 
renew_2023 <- read.csv("data/renew-use-2023.csv") |>
    rename(state = State, source = Energy_Source, usage = Renewable_Use_2023) |>
    mutate(year = 2023, usage = as.numeric(str_extract(usage, "\\d+")))
 
renew_total <- bind_rows(renew_2021, renew_2022, renew_2023)
 

```

ev-registrations-by-state-2023.csv — State-level EV registrations (messy text).
av-energy-price-2021-2023.csv — Total energy average price by state (keep 2023).
State boundaries from the rnaturalearth shown in lab session.
...
1, EV registrations by state (2023), choropleth with `geom_sf()`.
2, Total energy average price (2023), choropleth with `geom_sf()`.
scatterplot: `price_2023` (x) vs `ev_count` (y) with `geom_smooth(method = "lm")`.
